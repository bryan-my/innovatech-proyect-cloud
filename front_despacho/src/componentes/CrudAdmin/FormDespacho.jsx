...
const API_URL_DESPACHOS = import.meta.env.VITE_API_URL_DESPACHOS;
...
export const FormDespacho = ({ venta, onClose }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.put(
        `${API_URL_VENTAS}/api/v1/ventas/${venta.idVenta}`,
        jsonDataSales,
        {
          headers:{
      ...
      await axios.post(`${API_URL_DESPACHOS}/api/v1/despachos`, jsonData, {
        headers:{
          'Content-Type': 'application/json',
          'Accept': 'application/json'
    }
